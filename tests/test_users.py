...
def test_create_user(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "John Doe", "email": unique_email}),
...
def test_create_user_duplicate_email(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "First User", "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 201
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "Second User", "email": unique_email}),
        content_type="application/json",
    )
...
def test_get_user(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "Alice", "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 201, (
        f"Expected 201, got {response.status_code}"
...
def test_update_user(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "Bob", "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 201, (
        f"Expected 201, got {response.status_code}"
    ...
    response = test_client.put(
        f"/users/{user_id}/",
        data=json.dumps({"name": updated_name, "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 200
...
def test_update_user_duplicate_email(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "UserTwo", "email": "user2@example.com"}),
        content_type="application/json",
    )
    assert response.status_code == 201
    ...
    response = test_client.put(
        f"/users/{user2_id}/",
        data=json.dumps(
            {"name": "UserTwo Updated", "email": "user1@example.com"}
        ),
        content_type="application/json",
    )
    assert response.status_code == 409
...
def test_update_user_invalid_data(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "Test", "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 201
    user_id = response.json["id"]
...
def test_delete_user(test_client, db_session):
    ...
    response = test_client.post(
        "/users/",
        data=json.dumps({"name": "Charlie", "email": unique_email}),
        content_type="application/json",
    )
    assert response.status_code == 201, (
        f"Expected 201, got {response.status_code}"
